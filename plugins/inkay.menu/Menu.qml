...
Item {
  ...
  // TEMP-DEBUG: remove after diagnosing empty Apps.
  function debugApps() {
    try {
      var orderLen = root.itemOrder ? root.itemOrder.length : -1
      var itemKeys = 0
      try { itemKeys = Object.keys(root.items || {}).length } catch (e2) { itemKeys = -1 }
      var shellNull = (root.shell === null || root.shell === undefined) ? "yes" : "no"
      var shellPid = "n/a"
      try { shellPid = String(root.shell && root.shell.pluginId) } catch (e4) { shellPid = "threw:" + e4 }
      var hasKey = "n/a"
      try { hasKey = ("appLibrary" in root.shell) ? "yes" : "no" } catch (e5) { hasKey = "threw:" + e5 }
      var libIsNull = "n/a"
      try { libIsNull = (root.shell && root.shell.appLibrary === null) ? "null" : "not-null" } catch (e6) { libIsNull = "threw:" + e6 }
      var hasAppLib = (root.shell && root.shell.appLibrary) ? "yes" : "no"
      var summonType = "n/a"
      try { summonType = typeof (root.shell && root.shell.summon) } catch (e7) { summonType = "threw" }
      var mkinds = "n/a"
      try { mkinds = JSON.stringify((root.manifest && root.manifest.kinds) || null) } catch (e8) { mkinds = "threw:" + e8 }
      if (!root.appLibrary) return "appLibrary=null shellNull=" + shellNull + " shellPid=" + shellPid + " hasKey=" + hasKey + " libIsNull=" + libIsNull + " summon=" + summonType + " manifestKinds=" + mkinds + " rowsLoaded=" + root.rowsLoaded + " items=" + itemKeys + " order=" + orderLen
      var rows = []
      try { rows = root.appLibrary.sortedEntries("") || [] } catch (e3) { return "sortedEntries threw: " + e3 }
      var appCount = 0
      for (var i = 0; i < root.itemOrder.length; i++) {
        var ent = root.items[root.itemOrder[i]]
        if (ent && ent.kind === "app") appCount++
      }
      var hasAppsId = root.items && root.items["apps"] ? "yes" : "no"
      return "libRows=" + rows.length + " appItems=" + appCount + " items=" + itemKeys + " order=" + orderLen + " hasAppsId=" + hasAppsId + " provApps=" + (!!root.providersLoaded["apps"]) + " activeMenu=" + root.activeMenu + " rowsLoaded=" + root.rowsLoaded
    } catch (e) { return "debug threw: " + e }
  }
